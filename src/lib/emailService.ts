...
class EmailService {
  ...
  constructor(config?: Partial<EmailServiceConfig>) {
    this.resend = config?.resendClient || resendClient
    this.companyName = config?.companyName || "Shop Grocery"
    this.supportEmail = config?.supportEmail || process.env.SUPPORT_EMAIL || "support@shopgrocery.org"
    ...
    this.fromEmail = process.env.FROM_EMAIL || "info@shopgrocery.org"
    
    console.log("✅ Email service initialized with Resend")
  }

  // Send base email using Resend
  ...
  // 3. Order Confirmation - Fixed to send to both user and admin
  async sendOrderConfirmation(
    user: UserFromPrisma,
    order: OrderWithUser
  ): Promise<{ 
    customerEmail?: { id: string } | null
    adminEmail?: { id: string } | null 
  }> {
    const getItemPriceDisplay = (item: OrderItem) => {
      ...
      if (item.fixedPrice != null) {
        ...
        return `<p>Quantity: ${item.quantity}${unitDisplay} × ₦${price.toFixed(2)} = ₦${(item.quantity * price).toFixed(2)}</p>`
      }
    }
    ...
    try {
      ...
      if (this.supportEmail) {
        console.log("📧 Sending order notification to admin:", this.supportEmail)
    ...
    const content = `
      <div class="content">
        ...
        <div class="order-details">
          ...
          <p><strong>Processing Date:</strong> ${new Date(refund.processedAt).toLocaleDateString()}</p>
  ...
  async sendNewsletterWelcome(user: User): Promise<{ id: string } | null> {
    const content = `
      <div class="content">
    ...
    const content = `
      <div class="content">
        ...
        <div class="order-details" style="text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
          <h3 style="color: white; margin-bottom: 10px;">${promotion.title}</h3>
          <p style="font-size: 24px; font-weight: bold; margin: 10px 0;">${promotion.discount}</p>
          <p style="font-size: 18px;">Code: <strong>${promotion.code}</strong></p>
          <p style="font-size: 14px;">Valid until: ${new Date(promotion.expiresAt).toLocaleDateString()}</p>
          ${promotion.minOrderAmount ? `<p style="font-size: 12px;">*Minimum order: ₦${promotion.minOrderAmount}</p>` : ""}
        </div>
