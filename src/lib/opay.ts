...
class OpayService {
  ...
  async initializePayment(paymentData: OpayPaymentRequest): Promise<OpayPaymentResponse> {
    ...
    try {
      ...
      console.error('Opay service error details:', {
      ...
      if (error.response?.data) {
        ...
        throw new Error(`Opay payment initialization failed: ${error.message}`)
      }
    }
  }
  ...
  async verifyPayment(params: OpayVerifyPaymentParams): Promise<OpayVerifyResponse> {
    ...
    try {
      ...
      console.error('Opay verification error:', {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data
      })
      ...
      if (error.response?.data) {
        const errorData = error.response.data
        throw new Error(`Opay verification error: ${errorData.message || errorData.code || 'Unknown error'} (HTTP ${error.response.status})`)
      } else {
        throw new Error(`Opay payment verification failed: ${error.message}`)
      }
    }
  }
}
