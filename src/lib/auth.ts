...
export function requireAuth(
  handlerOrReq: ((req: NextRequest, context: { params?: Record<string, string> }) => Promise<NextResponse>) | NextRequest
): any {
...
export function requireAdmin(handlerOrReq: ((req: NextRequest) => Promise<NextResponse>) | NextRequest): any {
  ...
  return (async () => {
    try {
      ...
      try {
        const { getToken } = auth();
        const token = await getToken({ template: '_apptoken' });
        if (token) {
...
export async function checkModerator(req: NextRequest): Promise<NextResponse | null> {
  ...
  console.log('✅ Moderator access granted:', { email: user?.email, role: user?.role });
  return null;
}
...
export function requireRoles(roles: string[]) {
  return function(handler: (req: NextRequest) => Promise<NextResponse>): (req: NextRequest) => Promise<NextResponse> {
    return async (req: NextRequest) => {
      try {
        ...
        console.log('✅ Role access granted:', { email: user.email, role: user.role });
        return handler(req);
 ...
 */
export async function getCurrentAuthUser(req?: NextRequest): Promise<User | null> {
  try {
