...
export default function FreshFruitsPage() {
  ...
  const [sortBy, setSortBy] = useState<"featured" | "price-low" | "price-high" | "alphabetically" | "rating">("featured");
  ...
  useEffect(() => {
    actions.fetchProducts({ 
      ...
      status: 'ACTIVE',
      limit: 50,
      sortBy: sortBy === 'featured' ? 'createdAt' : 
             sortBy === 'alphabetically' ? 'name' : 
             sortBy.startsWith('price') ? 'fixedPrice' : 'rating',
      sortOrder: sortBy === 'price-high' ? 'desc' : 'asc'
    });
  }, [actions, sortBy]);

  // Filter for fresh fruit products (client-side fallback)
  const freshFruitProducts = products.filter(product => 
    product.isFruit && product.status === 'ACTIVE'
  );

  // Sort products (client-side fallback)
  const sortedProducts = React.useMemo(() => {
    return [...freshFruitProducts].sort((a, b) => {
      switch (sortBy) {
        case "price-low": 
  ...
  if (productsError) {
    return (
      <Container className="mx-auto px-4 py-8">
        <div className="flex justify-between items-center p-4 mb-6 shadow-md">
          <h1 className="text-xl text-orange-500 font-semibold">New Arrivals</h1>
        </div>
  ...
  if (freshFruitProducts.length === 0) {
    return (
      <Container className="mx-auto px-4 py-8">
        <div className="flex justify-between items-center p-4 mb-6 shadow-md">
          <h1 className="text-xl text-orange-500 font-semibold">Trending Products</h1>
        </div>
        <div className="text-center py-8">
