...
interface CategoryProductListProps {
  categorySlug: string;
  category?: any; // The category object passed from parent
}

function CategoryProductList({ categorySlug, category }: CategoryProductListProps) {
  ...
  useEffect(() => {
    ...
    if (categorySlug && category) {
      ...
      productActions.fetchProducts(fetchParams)
        .then((result) => {
  ...
  const showLoading = isLoading || (productsLoading && productsData.length === 0);
    ...
    return (
      <div className="text-center py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
          <p className="text-red-600 mb-4 font-medium">Failed to load products</p>
      ...
      <div className="text-center py-8">
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
          ...
          <p className="text-gray-500 text-sm">
            {category ? `No products available in "${category.name}" category.` : 'No products match your criteria.'}
          </p>
        </div>
      </div>
    );
