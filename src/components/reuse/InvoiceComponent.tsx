...
export function InvoiceComponent({
  ...
  }, [orderId, order?.paymentStatus, dispatch, pollingInterval]);
                  ...
                  {bankAccounts.length > 0 ? (
                    <>
                      ...
                      <div className="mt-4 bg-red-50 border-2 border-red-200 rounded-lg p-4">
                        ...
                        <div className="space-y-1 text-sm text-red-800">
                          ...
                          <p>
                            • Processing time: 1-2 business days after transfer
                          </p>
                          <p>• Keep your transfer receipt for reference</p>
                        </div>
                        <div className="mt-3 p-2 bg-red-100 rounded border-l-4 border-red-500">
                          <p className="text-xs text-red-700">
                            <strong>Note:</strong> Your order will only be
                            processed after payment confirmation. Please ensure
                            you transfer to one of the accounts above.
                          </p>
                        </div>
                      </div>
                    </>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      ...
                      <span className="text-yellow-800">
                        Bank account details are currently unavailable. Please
            ...
            <div className="mb-6 sm:mb-8">
                  ...
                  <table className="w-full border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <tbody>
                      {order.items?.map((item, index) => {
                        // Fix: getItemPrice expects a CartItem, but item is CheckoutItem (missing userId, updatedAt)
                        // We'll create a compatible object for getItemPrice
                        const cartItemLike = {
                          ...item,
                          userId: "", // or undefined if getItemPrice allows
                          updatedAt: new Date(), // or undefined if getItemPrice allows
                        };
                        const price = getItemPrice(cartItemLike);
              ...
              <div className="md:hidden space-y-3">
                {order.items?.map((item, index) => {
                  ...
                  const itemPrice = getItemPrice(cartItemLike);
            ...
            <div className="border-t border-gray-200 pt-4 sm:pt-6">
              <div className="text-center text-gray-600">
                ...
                <p className="mt-2 text-xs text-gray-400">
                  Invoice ID: {order.orderNumber || order.id} | Order ID: #
                  {order.id}
                </p>
              </div>
            </div>
